...
class Job {
    ...
    async execute(event_bus = null) {
            ...
            event_bus === null
                ...
                : {
                    emit_event_bus_result: (stage, result, event_bus) => {
                        const { error, code, signal, memoryUsage } = result;
        ...
        if (this.runtime.compiled) {
            ...
            compileTime = Date.now() - compileStartTime;
        ...
        if (!compile_errored) {
            ...
            runTime = Date.now() - runStartTime;
            run.memoryUsage = await this._getMemoryUsage(); // Track memory usage
        ...
        return {
            ...
            compileTime, // Add compile time to result
            runTime,     // Add run time to result
        };
    }
    
    async _getMemoryUsage() {
        // Replace with appropriate memory usage tracking logic
        // Example using `ps` command to get the RSS memory usage
        return new Promise((resolve, reject) => {
            exec(`ps -o rss= -p ${process.pid}`, (error, stdout) => {
                if (error) {
                    return reject(error);
                }
    ...
    _buildCommand() {
        // Construct your command based on runtime, args, files, etc.
        return `run your command here`;
    }
